      ...
      <div className="font-sans min-h-screen text-white pb-32 overflow-x-hidden flex flex-col lg:flex-row">
        ...
        <main className="lg:ml-72 flex-1 flex flex-col relative z-10 w-full">
          <header className="sticky top-0 z-40 flex h-20 w-full items-center px-6 lg:px-10 glass-panel border-b border-white/5">
            <h2 className="text-xl font-bold tracking-tight">Pilih Siswa untuk Cetak Rapor</h2>
          </header>
          <div className="p-6 lg:p-10 max-w-[1000px] mx-auto w-full space-y-6 flex-1">
            <div className="glass-card rounded-[2rem] p-8 border-primary/20 text-center mb-8 flex flex-col items-center">
              <h3 className="text-2xl font-black text-white mb-2">Cetak Rapor Kurikulum Merdeka</h3>
              <p className="text-slate-400 text-sm mb-6">Pilih siswa dari daftar di bawah ini atau cetak seluruh kelas sekaligus.</p>
              <Link to="/print/batch" className="flex items-center gap-2 px-8 py-4 text-white bg-gradient-to-r from-secondary to-primary hover:from-primary hover:to-secondary rounded-2xl font-bold shadow-lg shadow-primary/30 transition-all active:scale-95 text-lg">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
              {students.map(s => (
                <Link to={`/print/${s.id}`} key={s.id} className="glass-card rounded-[1.5rem] p-6 flex flex-col items-center gap-4 hover:scale-105 transition-all border-white/5 hover:border-primary/50 group">
                  ...
                  <div className="w-full mt-2 py-2 bg-white/10 rounded-lg text-center text-sm font-bold group-hover:bg-primary transition-colors flex items-center justify-center gap-2">
  ...
  return (
    <div className="font-sans overflow-x-hidden min-h-screen text-white bg-transparent print:bg-white pb-10">
      ...
      <main className="lg:ml-72 min-h-screen flex flex-col relative z-10">
        {/* Top App Bar */}
        <header className="sticky top-0 z-40 flex h-20 w-full items-center justify-between px-6 lg:px-10 glass-panel border-b border-white/5 no-print">
          <div className="flex items-center gap-4">
            <button className="lg:hidden p-2 text-white bg-white/5 rounded-full backdrop-blur-md">
              <span className="material-symbols-outlined" data-icon="menu">menu</span>
            </button>
            <h2 className="text-xl font-bold text-white tracking-tight hidden sm:block">Cetak Raport</h2>
          </div>
          <div className="flex items-center gap-4">
        ...
        <div className="flex-1 overflow-auto p-6 lg:p-10 no-print pb-24 lg:pb-10 w-full max-w-[1440px] mx-auto">
          <div className="space-y-8">
            ...
            {/* Action Header */}
            <div className="glass-card rounded-[2rem] p-6 sm:p-8 flex flex-col lg:flex-row lg:items-center justify-between gap-6 border-white/5 relative overflow-hidden">
              ...
              <div className="relative z-10">
                <h2 className="text-2xl font-black text-white tracking-tight mb-2 flex items-center gap-3">
                  <span className="material-symbols-outlined text-secondary text-3xl">preview</span>
                  {isBatch ? 'Preview Batch: Semua Murid' : `Preview: ${student.name}`}
                </h2>
                <p className="text-slate-400 font-medium text-sm">Pratinjau dokumen sebelum dicetak ke format PDF atau printer fisik.</p>
              </div>
              <div className="flex flex-wrap items-center gap-3 relative z-10">
                {!isBatch && (
                  <Link to={`/editor/${id}`} className="flex items-center gap-2 px-5 py-3 bg-white/5 hover:bg-white/10 text-white rounded-xl font-bold transition-colors border border-white/10 backdrop-blur-md">
                    <span className="material-symbols-outlined text-lg" data-icon="arrow_back">arrow_back</span>
                    Edit Kembali
                  </Link>
                )}
                <button onClick={handlePrint} className="flex items-center gap-2 px-6 py-3 text-white bg-gradient-to-r from-secondary to-primary hover:from-primary hover:to-secondary rounded-xl font-bold shadow-lg shadow-primary/30 transition-all active:scale-95">
